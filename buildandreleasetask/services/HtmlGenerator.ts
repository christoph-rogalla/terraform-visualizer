...
export default class HtmlGenerator {
  ...
  generateHtmlFrom(changes: ResourceChanges[]) {
    ...
    handlebars.registerHelper('diffHighlight', function (before, after, mode) {
      // Quick exit
      if (before === undefined && after === undefined) {
        return new handlebars.SafeString('<pre><em>No data</em></pre>');
      }
      ...
      function diffKeys(b: any, a: any, path = '') {
        ...
        if (!isPlainObject(b) && !isPlainObject(a)) {
          ...
          if (JSON.stringify(b) !== JSON.stringify(a)) {
            if (b === undefined) result.added.push(path || '__root__');
            else if (a === undefined) result.removed.push(path || '__root__');
            else result.changed.push(path || '__root__');
          }
          return result;
        }
      ...
      function renderValue(val: any, currentPath: any): string {
      ...
      function renderObject(obj: any, path = '') {
        ...
        const rendered = keys.map(k => {
          ...
          const inBefore = (() => {
            try {
              if (!isPlainObject(beforeObj)) return false;
          ...
          const inAfter = (() => {
            try {
              if (!isPlainObject(afterObj)) return false;
              const parts = fullPath.split('.');
              let cur = afterObj;
              for (const p of parts) {
                if (!isPlainObject(cur) && !Array.isArray(cur)) return false;
      ...
      function getAtPath(obj: any, path: any) {
