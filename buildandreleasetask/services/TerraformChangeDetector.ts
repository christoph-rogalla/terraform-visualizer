...
export default class TerraformChangeDetector {
  detectChanges(plan: TerraformPlan) {
    console.log("Analyzing Terraform plan changes...");
    // filter out read or no operation actions
    let filtered = plan.resource_changes.filter(change => !change.change.actions.includes(ActionType.NONE) && !change.change.actions.includes(ActionType.READ));
